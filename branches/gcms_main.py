...
def main():
    train_data = pd.read_csv("../data/gcms_train.csv")
    test_data = pd.read_csv("../data/gcms_test.csv")
    all_features = pd.concat((train_data.iloc[:,:], test_data.iloc[:,:]))
    numeric_features = all_features.dtypes[all_features.dtypes != 'object'].index[:-1]
    ...
    def get_net():
        ...
        net = nn.Sequential(nn.Linear(in_features,209),nn.ReLU(),
    ...
    def train(net, train_features, train_labels, test_features,test_labels,
        ...
        # Save the model
        torch.save(net.state_dict(), '../models/MLP-G-4.pt')
    ...
    def train_and_pred(train_features, test_features, train_labels, test_data, num_epochs, lr, weight_decay, batch_size):
        ...
        # submission.to_csv('../results/gcms_submission.csv',index = False)
        # submission.to_csv('../results/gcms_submission_pca.csv',index = False)
